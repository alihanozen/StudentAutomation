...
public class matogretmen extends ogretmen implements interogretmen,ortak {
	Connection con;
	static Scanner scan = new Scanner(System.in);

	public matogretmen(String name, String surname, int id, String password) {
	...
	public static void ogrencigoster(Connection con) {
		...
		try {
			...
			while (res.next()) {
				...
				if (d1.equalsIgnoreCase("matematik")) {
					System.out.println(adi + " " + soyadi + " " + no + " " + d1);
				}
				if (d2.equalsIgnoreCase("matematik")) {
	...
	public static void notgir(Connection con, int id) {
		...
		try {
			...
			while (res.next()) {
				...
				float a;
				if (as.equalsIgnoreCase("matematik")) {
				...
				if (bs.equalsIgnoreCase("matematik")) {
					System.out.println("Matematik Notu: ");
					a = scan.nextFloat();
